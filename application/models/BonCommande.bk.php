...
class BonCommande extends CI_Model {
	...
	public function getlistBC() {
		...
		$this->db->select('bc.id AS idBC, bc.numero, bc.date, f.raison_social as fournisseurs_raison_social');
	...
	public function getAllBcExpand($Id = 0) {
		...
		$this->db->select('bc.id as idBC,numero, UNIX_TIMESTAMP(date)*1000 as date,f.raison_social as fournisseurs_raison_social');
